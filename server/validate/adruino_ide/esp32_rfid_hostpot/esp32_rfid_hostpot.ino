      ...
      for (int i = 0; i < rfid.uid.size; i++) {
       ...
       Serial.println();
        if (rfid.uid.uidByte[0] == 0x62 && rfid.uid.uidByte[1] == 0x48 && rfid.uid.uidByte[2] == 0x1B && rfid.uid.uidByte[3] == 0x51) {
          ...
          String id = "Banana2";
          WriteRFID(rfidUID,id);
        } //else if (rfid.uid.uidByte[0] == 0x62 && rfid.uid.uidByte[1] == 0xF2 && rfid.uid.uidByte[2] == 0x62 && rfid.uid.uidByte[3] == 0x51) { 
          //Serial.println("เครื่อง : Banana 3");
          //String id = "Banana3";
          //WriteRFID(rfidUID,id);
...
void WriteRFID(String uuid, String idesp) {
  ...
  WiFiClient client;
  HTTPClient http;
  if (http.begin(client, "http://" + String(host) + ":" + String(port) + "/writeuuid/" + _uuid + "/" + _idesp)) {
