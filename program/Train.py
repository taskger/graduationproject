...
def StartGrabbing():
    ...
    if not round == 0 : 
        ...
        b = 0
        save_frame_on_rgb()
        banana = int(ui.lineEdit_amountBanana.text())
        print('test',ui.lineEdit_nFrame.text(),banana)
...
def SaveJpeg():
    ...
    for i in range(1):
        ...
        if response.status_code == 200:
            ...
            setnamesetting = 'None'
            setnamebanana = 'None'
        file_path = 'F:\\proend\\Python\\BasicDemo\\img\\' + str(year) + "-" + str(month)  + "-" + str(day) +" " + str(hour) + ";" + str(min)  + ";" + str(sec)  + ";" + str(microsec) + ' ' + str(setnamebanana) +  " " + str(setnamesetting) + " " + str(nFrameNum) + ".jpg"
        #สร้างตำแหน่งในการเก็บไฟล์ภาพโดยที่จะตั้งเป็นวันเวลา-ลำดับของกล้วย-แสงทีี่ใช้-ลำดับของเฟรม
